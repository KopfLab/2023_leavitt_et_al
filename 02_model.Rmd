...
calculate_model_output <- function(n_rings, XTD, alpha_W, x, alpha_NADPH, alpha_FMN_vs_NADPH, k) {
  alpha_ETC <- 1
  alpha_x <- alpha_W
  f_W <- f_W(n_rings, x)
  ...
  f_x <- f_x(n_rings, x)
  alpha_W * f_W + alpha_x * f_x + alpha_ETC * alpha_NADPH * (f_NADPH + alpha_FMN_vs_NADPH * f_FMN) / (1 + k * (1 - XTD))
}

# non-linear least squares fit of model
#start_values <- list(alpha_NADPH = 0.1, alpha_FMN_vs_NADPH = 0.2, k = 0.1)
set.seed(123)
start_values <- list(alpha_NADPH = runif(1), alpha_FMN_vs_NADPH = runif(1), k = runif(1))
message("Fitting with random start values: ")
print(start_values)
fit_model_by_nls <- function(alpha_W, x) {
  nls(
    ~(calculate_model_output(n_rings, XTD, alpha_W, x, alpha_NADPH, alpha_FMN_vs_NADPH, k) - alpha) * 1000, 
...
fits_wide <- fits %>%
  ...
  transmute(
    ...
    alpha_FMN_vs_NADPH = estimate_alpha_FMN_vs_NADPH,
    alpha_FMN_vs_NADPH_se = std.error_alpha_FMN_vs_NADPH,
    alpha_FMN_vs_NADPH_ci_2.5 = `2.5 %_alpha_FMN_vs_NADPH`,
    alpha_FMN_vs_NADPH_ci_97.5 = `97.5 %_alpha_FMN_vs_NADPH`,
    k = estimate_k,
    k_se = std.error_k
  ) 
head(fits_wide)
...
fits_alphas <- 
  ...
  mutate(
    ...
    m_alpha = calculate_model_output(
      n_rings, XTD, alpha_W, x, alpha_NADPH, alpha_FMN_vs_NADPH, k
    )
  ) 
...
fits_alphas %>% 
  filter(!is.na(alpha)) %>%
  group_by(model_id, x, x_fct, alpha_W, alpha_NADPH, alpha_FMN_vs_NADPH, k, resid.permil) %>%
  summarize(
