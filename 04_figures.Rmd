...
fits_sum <- readr::read_rds("cache/fits_sum.rds")
...
p_model_alphas <-
  fits_sum %>%
  ggplot() +
  aes(alpha_NADPH, alpha_FMN_NADPH, color = factor(alpha_W), shape = x_fct, linetype = x_fct) +
  geom_smooth(map = aes(color = NULL), method = "loess", formula = y ~ x, color = "black", span = 2, se = FALSE) +
  geom_errorbar(
    map = aes(ymin = alpha_FMN_NADPH - alpha_FMN_NADPH_se, ymax = alpha_FMN_NADPH + alpha_FMN_NADPH_se),
    width = 0, linetype = 1) +
  geom_errorbarh(
